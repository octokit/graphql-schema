...
schema.json = JSON.parse(
  readFileSync(pathJoin(__dirname, "..", "schema.json"), "utf8")
);
const branchName = `cron/fixtures-changes/${new Date()
  .toISOString()
  .substr(0, 10)}`;
let lastCommitSha;
...
request("/user")
  .then((response) => {
    ...
    console.log(
      `🤖  Signed in as ${login}. Creating a pull request on ${repoName}`
    );

    console.log(
      `🤖  Looking for last commit sha of ${repoName}/git/refs/heads/master`
    );
  ...
  .then((response) => {
    ...
    console.log(
      `🤖  Creating new branch: ${branchName} using last sha ${lastCommitSha}`
    );
  ...
  .then((response) => {
    ...
    return request(
      `PUT /repos/:owner/:repo/contents/schema.graphql?ref=${branchName}`,
      {
        path: "schema.graphql",
      ...
      .then(() => {
        ...
        return request(
          `PUT /repos/:owner/:repo/contents/schema.json?ref=${branchName}`,
          {
            path: "schema.json",
            content: Buffer.from(JSON.stringify(schema.json, null, 2)).toString(
              "base64"
            ),
            sha: jsonSha,
